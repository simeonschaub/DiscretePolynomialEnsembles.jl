...
# ╔═╡ ca2e357c-f6b5-11f0-80e3-bd03983e7fe2
begin
	using Revise
	let p = dirname(pwd())
...
# ╔═╡ a440d714-ddd6-46b6-856b-dae30e0a907a
using MathLink, LinearAlgebra

# ╔═╡ 24a5c882-3d42-4325-a1ba-9971ef8daf2c
using WGLMakie

# ╔═╡ ee7ef62b-21ae-4e51-88c3-62d5569c0256
M = 10

# ╔═╡ 8fd30e77-e115-4ffe-b18a-b40728d0318c
q = 1.3

# ╔═╡ 075b7a08-4080-4dd0-b15f-bf493df117a5
α = 2

# ╔═╡ dc16f80e-74e4-4d93-8449-116af6efefc3
β = 4

# ╔═╡ a40ebda6-42b6-4dbb-8662-f04707a211e2
γₙ = q^-(M + 1)

# ╔═╡ 12178665-4fb4-4536-a2f2-c6f83d15f517
δ = -2

# ╔═╡ 6bd42d76-3960-40c0-b364-89d295180c56
νₙ(y) = q^-y + δ * γₙ * q^(y + 1)

# ╔═╡ 37c537bc-c115-401b-9738-314da02186c5
qpochhammer(a, n; q) = prod(k -> 1 - a * q^k, 0:(n - 1); init = one(a))

# ╔═╡ 65fa0756-938d-4a91-a6cb-445e9a5463a8
let x = -2:(M + 2)
	lines(x, w.(x))
end

# ╔═╡ e95c5ece-93d0-4603-b5b6-1e38e1defd9b
l = let weights = w.(0:M)
	Lanczos(y -> weights[y + 1], 0:M; ν = νₙ)
end

# ╔═╡ 43dc7733-fc32-443b-9c86-04f3941d8559
let
	fig = Figure()
	ax = Axis(fig[1, 1]; limits = ((-1, M + 1), nothing))
	for n in 0:3
		lines!(ax, -1..(M + 1), normalize(l[n]) ∘ νₙ)
		scatter!(ax, 0:M, (normalize(l[n]) ∘ νₙ).(0:M))
	end
	fig
end

# ╔═╡ 58c84a1b-45cc-4c6d-b0b5-b4868b1a1095
let
	fig = Figure()
	ax = Axis(fig[1, 1]; limits = (extrema((νₙ(-1), νₙ(M + 1))), nothing))
	for n in 0:3
		lines!(ax, (..)(extrema((νₙ(-1), νₙ(M + 1)))...), y -> normalize(l[n])(y))
		scatter!(ax, νₙ.(0:M), normalize(l[n]).(νₙ.(0:M)))
	end
	fig
end

# ╔═╡ c266a6de-23cb-4973-bd3b-14a264602552
map(CartesianIndices((0:M, 0:M))) do I
	i, j = Tuple(I)
	sum(0:M) do y
		l.w(y) * normalize(l[i])(νₙ(y)) * normalize(l[j])(νₙ(y))
	end
end

# ╔═╡ fd8a19bf-04bd-4de6-9aa1-9c94bd01c584
kernel = map(CartesianIndices((0:M, 0:M))) do I
	Kernel(l, 3)(Tuple(I)...)
end

# ╔═╡ de05bbfa-9a06-429c-ae35-83fcf2a790f6
eigen(kernel)

# ╔═╡ a552ca74-5111-4908-8d11-c2377846c70c
kernel^2 ≈ kernel

# ╔═╡ ec8b643e-8db1-4f10-8e3c-a5ab0e98a3b4
w(y) = (0 ≤ y ≤ M) * prod(qpochhammer.([α * q, β * δ * q, γₙ * q, γₙ * δ * q], y; q)) *
	(1 - γₙ * δ * q^(2y + 1)) / (
	prod(qpochhammer.([q, inv(α) * γₙ * δ * q, inv(β) * γₙ * q, δ * q], y; q)) *
	(α * β * q)^y * (1 - γₙ * δ * q)
	)

# ╔═╡ 82e6b900-c769-4ea5-b6d2-b5b6b34afaed
# ╠═╡ disabled = true
#=╠═╡
w(y) = weval(
	prod(W"QPochhammer".([α * q, β * δ * q, γₙ * q, γₙ * δ * q], q, y)) *
	(1 - γₙ * δ * q^(2y + 1)) / (
	prod(W"QPochhammer".([q, inv(α) * γₙ * δ * q, inv(β) * γₙ * q, δ * q], q, y)) *
	(α * β * q)^y * (1 - γₙ * δ * q)
	)
)
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "5a1c7dcaf0003f0be8c678449b7717236f2af407"
