...
# ╔═╡ 27b3c442-e952-11ef-2cf0-d7990156824f
using WGLMakie, Distributions

# ╔═╡ f6dbdabe-6a35-4bea-bc23-63d61c5d7970
using FHist, RandomMatrices, ForwardDiff

# ╔═╡ ce0ec3f8-eabd-4144-a0e1-9300da42aa7c
using OhMyThreads, Random
...
# ╔═╡ 2808d9b4-9b1f-49b5-a127-1ee3968dd260
...
# ╔═╡ 021d30b5-ac95-4636-adbd-0e62e1fef61f
p = 0.5
...
# ╔═╡ be72b860-b249-4797-a157-0598819da3b7
let
	global fig = Figure(; size = (600, 600))
	ax0 = nothing
	for (I, p) in pairs([0.1 0.25; 0.5 0.8])
		i, j = Tuple(I)
		
		ax = Axis(fig[i, j]; title = L"p = %$p", aspect = DataAspect(), limits = (0, 1000, 0, 1000))
		ax0 === nothing || linkaxes!(ax, ax0)
		ax0 = ax
		j == 1 || hideydecorations!(ax; grid = false)
		i == 2 || hidexdecorations!(ax; grid = false)
		tightlimits!(ax)
		
		T = accumulate_growth(rand(Bernoulli(p), 1000, 1000))
		heatmap!(ax, T; colorrange = map(t -> (0, t), t), highclip = :transparent, colormap = Reverse(:inferno))

		q = 1 - p
		lines!(ax, 0..1000, map(t -> (l -> (q * t > p * l) ? (√(l + t) - √(q * l))^2 / p : t), t); color = :red, linewidth = 2)
		lines!(ax, 0..1000, l -> p / q * l, color = :green)
	end
	Label(fig[1, :, Top()], map(t -> L"t = %$t", t); padding = (0f0, 0f0, 16f0, 0f0))
	fig
end

# ╔═╡ ef7a697e-489d-4eb9-8eaf-1f5b62b6e0f1
for i in 0:1000
	t[] = i
	sleep(0.005)
end

# ╔═╡ 016e1d80-9d78-4c6d-871d-fb1924e31c9f
σ(x, y, p, q) = ((p * q)^(1/6) / (x * y)^(1/6)) * (√(p * x) + √(q * y))^(2/3) * (√(q * x) - √(p * y))^(2/3)

# ╔═╡ fba22b25-6b81-48ae-918a-db61ab42a1f2
let
	hist = Hist1D(; counttype = Int, binedges = range(-5, 2; length = 51))
	k, l = 5000, 2000
	@tasks for _ in 1:5000
		@local (W, T) = (Matrix{Bool}(undef, l, k), Matrix{Int}(undef, l, k))
		p = 0.5
		q = 1 - p
		rand!(Bernoulli(p), W)
		t = accumulate_growth!(T, W)[end, end]
		ξ = (t - ((k - l) * p + 2√(l * k * p * q))) / σ(k / t, l / t, p, q) / t^(1/3)
		atomic_push!(hist, ξ)
	end
	fig = Figure()
	ax = Axis(fig[1, 1])
	hist!(ax, normalize(hist))
	lines!(ax, -5..2, x -> ForwardDiff.derivative(x -> cdf(TracyWidom(2), x), x); color = Cycled(2), linewidth = 3)
	fig
end

# ╔═╡ 378500fe-6706-42db-b1a9-02ea5fd058b3
let
	hist = Hist1D(; counttype = Int, binedges = range(-5, 2; length = 51))
	k, l = 5000, 2000
	@tasks for _ in 1:5000
		...
		q = 1 - p
		rand!(Bernoulli(p), W)
		let (k, l) = (rand(4950:5000), rand(1950:2000))
			t = accumulate_growth!(T, W)[l, k]
			ξ = (t - ((k - l) * p + 2√(l * k * p * q))) / σ(k / t, l / t, p, q) / t^(1/3)
			atomic_push!(hist, ξ)
		end
	end
...
# ╔═╡ ccbfe4b4-c287-485c-82f1-bfffa16f3480
# ╠═╡ disabled = true
# ╠═╡ skip_as_script = true
#=╠═╡
record(fig, "growth_animation.mp4", 0:1:1000;
        framerate = 30) do i
    t[] = i
end
  ╠═╡ =#

# ╔═╡ 91c34f31-60ee-4fa8-ba30-fe1716b008e3
t2 = Observable(0.0)

# ╔═╡ 91269a6a-462f-4941-9118-4c0ad0cc90b1
let
	global fig2 = Figure()
	ax = Axis(fig2[1, 1]; aspect = DataAspect(), limits = (0, 1000, 0, 1000), title = map(t -> L"t = %$t", t2))

	T = accumulate_growth(rand(Exponential(), 1000, 1000); offset = false)
	heatmap!(ax, T; colorrange = map(t -> (0, t), t2), highclip = :transparent, colormap = Reverse(:inferno))
	#lines!(ax, 0..1000, map(t -> (x -> t * exp(-log(x) / log(t^(4/3)))), t2))
	
	fig2
end

# ╔═╡ 8d3067ab-c82e-4a3d-9c93-0146c9c0f0a4
t2[] = 50

# ╔═╡ a38ceacb-366f-4614-aeb3-341aa9c57351
for i in 0:2000
	t2[] = i
	sleep(0.005)
end

# ╔═╡ fe338e22-4bc1-4d8f-b9d7-0c56eb523e90
# ╠═╡ disabled = true
#=╠═╡
record(fig2, "growth_exp_meixner_animation.mp4", 0:2:2000;
        framerate = 30) do i
    t2[] = i
end
  ╠═╡ =#

# ╔═╡ 695e9d79-8c84-4ef6-a0ba-63902433c88d
let
	fig2 = Figure(; size = (650, 200))
	ax = Axis(fig2[1, 1]; limits = (1, 5000, 10, 100), title = map(t -> L"t = %$t", t2), xscale = log, yscale = log)

	T = accumulate_growth(rand(Exponential(), 5000, 100); offset = false)
	heatmap!(ax, T; colorrange = map(t -> (0, t), t2), highclip = :transparent, colormap = Reverse(:inferno))
	lines!(ax, 1..5000, map(t -> (x -> t * (exp(-(log(x) / 2log(t))))^2), t2))
...
manifest_format = "2.0"
project_hash = "fc22f2b3c13f121d36ecf11eb4399caccf05e4ca"
