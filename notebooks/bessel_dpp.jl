...
using SpecialFunctions, WGLMakie, Bonito
...
kernel = tmap(CartesianIndices((-cutoff:cutoff, -cutoff:cutoff))) do I
    Kernel(b)(Tuple(I)...)
end
...
let
	fig, ax, hm = heatmap(-cutoff:cutoff, -cutoff:cutoff, kernel)
	Colorbar(fig[1, 2], hm)
	fig
end

# ╔═╡ 38a18fe3-fa29-45c1-87bd-0a38ace249bb
function randDPPseq!(K; warn = false)
	𝓘 = Int64[]
	n = size(K, 1)
	for j in 1:n
		warn && !(0 ≤ K[j,j] ≤ 1) && @warn "K[$j, $j] == $(K[j,j])"
...
[sources]
DiscretePolynomialEnsembles = {path = "/home/simeon/.julia/dev/PolynomialEnsembles"}
