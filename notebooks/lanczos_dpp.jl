...
# ╔═╡ ce65777a-4fbc-4629-aaec-0f95ef3b4905
using DimensionalData

# ╔═╡ 89c13ec1-ac93-45c6-8816-cbeddba7fadf
...
# ╔═╡ 5f04c036-8019-46b2-bf5a-8b564a4d020e
Dict((i, j) => det(kernel[[i + 1, j + 1], [i + 1, j + 1]]) for i in 0:cutoff, j in 0:cutoff if i < j)
...
bessel = tmap(CartesianIndices((-M:(N - 1), -M:(N - 1)))) do I
...
let colorrange = (-0.2, 1)
	...
	heatmap(fig[1, 2], -M:(N - 1), -M:(N - 1), bessel; colorrange, axis = (; title = "Bessel Kernel"))
	Colorbar(fig[1, 3], hm)
	fig
end
...
begin
	...
	@tasks for j in 1:10000
		for i in 1:50
			L = rand(Truncated(Poisson(θ), 0, M * N))
			p = randperm(L)
			P = rs_norecord(p)
			while YoungTableaux.nrows(P) > M || YoungTableaux.ncols(P) > N
				L = rand(Truncated(Poisson(θ), 0, M * N))
				p = randperm(L)
				P = rs_norecord(p)
			end
...
let
	...
	stairs!(ax, (-1:cutoff+1) .- M .+ 1.5, diff([0;y; 1; 1]); color = :yellow, linewidth = 2, linestyle = :dot, label = "Fredholm Det")
...
# ╔═╡ 3bcd544e-2238-479f-b9b7-ca3c988ca2dc
begin
...
manifest_format = "2.0"
project_hash = "ce5d64dfe80d8d33cb2947968e73699cac14a180"
