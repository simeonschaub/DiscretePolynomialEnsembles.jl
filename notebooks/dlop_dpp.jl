...
# ╔═╡ 411ab969-e9a5-4446-bc8c-7f8729ad6a3c
...
# ╔═╡ 5fe1abd4-2c41-4f82-9369-06f55be84f3e
N = 2M - 1

# ╔═╡ 9e3ce14d-8134-4271-973b-73410d6ebe25
d = DiscreteLegendre(; N)

# ╔═╡ 373a530e-0d79-4fa3-bf3e-3efa8e957fcd
kernel = tmap(CartesianIndices((0:N, 0:N))) do I
    Kernel(d, M)(Tuple(I)...)
end
...
let
	fig, ax, hm = heatmap(0:M, 0:M, kernel)
	Colorbar(fig[1, 2], hm)
	fig
end
...
# ╔═╡ 7ae931b7-0664-43bb-a0fa-6561c31794b6
parts = Partition.(P for m in 0:M^2 for P in partitions(m) if length(P) <= M && get(P, 1, 0) <= M)
...
let
	...
	x = -cutoff:cutoff
	y = map(x) do k
		det(I - kernel[(k:cutoff) .+ (cutoff + 1), (k:cutoff) .+ (cutoff + 1)])
	end
	stairs!(ax, ((1 - cutoff):cutoff) .+ 0.5, diff(y); color = :yellow, linewidth = 2, linestyle = :dot, label = "Fredholm Det")
...
# ╔═╡ 7e5ad81d-eaf6-4963-b40d-3070ced23b36
# ╠═╡ disabled = true
#=╠═╡
begin
	hists2 = [Hist1D(; counttype = Int, binedges = -0.5:40.5) for _ in 1:M, _ in 1:10]
	@tasks for _ in 1:10000
		#@local K = Matrix{BigFloat}(undef, 2cutoff + 1, 2cutoff + 1)
		for i in 1:10
			#copyto!(K, kernel)
			#h = randDPPseq!(K)
			h = randDPPproj(Y) .- 1
			P = Partition(reverse(h) .- M .+ eachindex(h))'
...
manifest_format = "2.0"
project_hash = "35ac102a203580230063b11ba42b8c29f6e00a7c"
