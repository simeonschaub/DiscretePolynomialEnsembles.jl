...
# ╔═╡ e91c5ad2-d59e-40be-bbe0-fffd882afcfd
begin
	using Revise
	let p = dirname(pwd())
...
# ╔═╡ 0b8963b8-e7e3-11f0-a873-1d18d6a9bedd
using WGLMakie, Bonito

# ╔═╡ 00b8e1d8-dc84-44b1-81d1-31a9890a0b3c
using LogExpFunctions, SpecialFunctions

# ╔═╡ 484583e5-b6ea-4d1e-b264-3119c62eca8b
using OhMyThreads

# ╔═╡ 672fb9ac-dacd-4985-8137-23fe0ef332bd
using Mooncake

# ╔═╡ 283c7c20-86dc-457c-894b-83c773712cca
using LinearAlgebra

# ╔═╡ e53049b9-e20d-4293-9e43-7f7d0086017a
...
using Random, Distributions, YoungTableaux

# ╔═╡ cd07a06e-44fc-46f1-8784-e0e0b1c674b0
using DimensionalData

# ╔═╡ 83d30418-8d79-40ac-8a5f-cfade0857aeb
using Combinatorics: combinations

# ╔═╡ b3910735-bf5e-4b88-9056-3a91fcbe94e5
using Atomix: @atomic

# ╔═╡ 48782588-2f01-4a71-bb5d-c835a2df81b9
Page()

# ╔═╡ 2fa25e26-bda5-435a-8305-2d15ade4697e
...
# ╔═╡ 8da24972-8107-4a20-b360-33de07c76516
cutoff = 2M - 1

# ╔═╡ ffd4b433-14f7-48b1-90e3-44813037aab4
w(x) = exp(xlogy(x, θ) - θ - 2loggamma(x + 1))

# ╔═╡ c7396516-4a82-4a1a-a7df-a72524582d49
function _randDPPproj(Y)
	r = size(Y, 2)
	𝓘 = zeros(Int, r)
	sensitivities = zeros(r)
	for k in 1:r
		p = mean(abs2.(Y), dims=2)
		𝓘[k] = rand(Categorical(vec(p)))
		sensitivities[k] = p[𝓘[k]]
		Y = (Y * qr(Y[𝓘[k], :]).Q)[:, 2:end]
	end
	π = sortperm(𝓘; rev = true)
	return 𝓘[π], sensitivities[π]
end

# ╔═╡ b6e5b275-2db8-4d0f-a8c8-8fae8f4b7350
begin
	@noinline fake_grad(x, y) = x
	Mooncake.@is_primitive Mooncake.DefaultCtx Mooncake.ReverseMode Tuple{typeof(fake_grad), Any, Any}
	function Mooncake.rrule!!(::Mooncake.CoDual{typeof(fake_grad)}, x::Mooncake.CoDual, y::Mooncake.CoDual)
		function fake_grad_adjoint(dy)
		    return Mooncake.NoRData(), Mooncake.NoRData(), dy
		end
		return Mooncake.CoDual(x.x, y.dx), fake_grad_adjoint
	end
end

# ╔═╡ e89fb6d6-7aae-43b9-b385-7cba12c370e5
function _randDPPseq!(K)
	𝓘 = Int64[]
	sens = Float64[]
	n = size(K, 1)
	for j in 1:n
...
# ╔═╡ f58d2d57-dae3-43bd-b9fe-ef744b916a28
begin
	print_pb(x) = x
	Mooncake.@is_primitive Mooncake.DefaultCtx Mooncake.ReverseMode Tuple{typeof(print_pb), Any}
	function Mooncake.rrule!!(::Mooncake.CoDual{typeof(print_pb)}, x::Mooncake.CoDual)
		function dfoo_adjoint(dy)
			@show x.x x.dx dy
		    return Mooncake.NoRData(), dy
		end
		return x, dfoo_adjoint
	end
end

# ╔═╡ 16032038-ad15-4dc5-8e8a-7401466966c2
begin
	...
	Mooncake.@is_primitive Mooncake.DefaultCtx Mooncake.ReverseMode Tuple{typeof(det), AbstractMatrix{<:LinearAlgebra.BlasFloat}}
	function Mooncake.rrule!!(::Mooncake.CoDual{typeof(det)}, x::Mooncake.CoDual{<:AbstractMatrix{<:LinearAlgebra.BlasFloat}})
		function det_adjoint(dy)
			x.dx .+= dy .* adjugate(x.x)'
		    return Mooncake.NoRData(), Mooncake.NoRData()
		end
		return Mooncake.CoDual(det(x.x), Mooncake.NoFData()), det_adjoint
	end
end

# ╔═╡ faa3aa80-16f1-405a-82cf-9f59935e35ee
backend = DI.AutoMooncake()

# ╔═╡ 7960c86a-35f2-4a82-98fb-d6fd8ef0ec72
DI.value_and_gradient(backend, [1., 2., 3.]) do x
	fake_grad([1.0], [2x[1] + x[3]])[]
end

# ╔═╡ f03a1b94-c8ef-4085-92ae-5c6cec066362
import SciMLBase, ADTypes, OptimizationOptimisers, Optimization, OptimizationOptimJL

# ╔═╡ 9f008989-c0c4-40ad-97d8-d65d0a3ce189
let
	x = rand(3, 3)
	DI.value_and_gradient(det, backend, x), DI.value_and_gradient(det, ADTypes.AutoForwardDiff(), x)
end

# ╔═╡ 1c839b69-9d6a-4c73-b363-13ffbe5ee2d5
bessel = tmap(CartesianIndices((-M:(M - 1), -M:(M - 1)))) do I
    Kernel(BesselJ(θ))(Tuple(I)...)
end

# ╔═╡ 331be8cb-6779-45d5-a004-3341c738d41c
...
begin
	...
	@tasks for _ in 1:10000
		for i in 1:50
			N = rand(Truncated(Poisson(θ), 0, M^2))
			p = randperm(N)
			P = rs_norecord(p)
			while YoungTableaux.nrows(P) > M || YoungTableaux.ncols(P) > M
				N = rand(Truncated(Poisson(θ), 0, M^2))
				p = randperm(N)
				P = rs_norecord(p)
			end
			λ = YoungTableaux.ncols.(Ref(P), 1:M)
			atomic_push!.(@view(hists2[:, i]), λ)
		end
	end
...
# ╔═╡ d745603a-898a-47f8-9976-9ef13ff117ea
let cutoff = cutoff, M = M
	μ1 = getindex.(hists2_errors[1], 2)[1:(M + 1)]
	σ1 = hypot.(getindex.(hists2_errors[1], 3)[1:(M + 1)], 0.5e-5)
	μ2 = getindex.(hists2_errors[2], 2)[1:(M + 1)]
	σ2 = hypot.(getindex.(hists2_errors[2], 3)[1:(M + 1)], 0.5e-5)
	global function loss(wᵢ)
		w(x) = wᵢ[x + 1]^2
		l = Lanczos(w, 0:cutoff)
		kernel = map(CartesianIndices((0:cutoff, 0:cutoff))) do I
		    Kernel(l, M)(Tuple(I)...)
		end
		y1 = map((cutoff - M):cutoff) do k
			det(I - kernel[(k:cutoff) .+ 1, (k:cutoff) .+ 1])
		end
		d1 = diff(push!(y1, 1))

		y2 = map((cutoff - M - 1):cutoff) do k
		...
		return -sum(x -> logpdf(Normal(μ1[x + 1], σ1[x + 1]), d1[x + 1]), 0:M) - sum(x -> logpdf(Normal(μ2[x + 1], σ2[x + 1]), d2[x + 1]), 0:M)
		#norm(μ1 - d1) + norm(μ2 - d2)
	end
end

# ╔═╡ c764516c-dc4a-4c8b-a544-1df7bdee2e5e
loss(w.(0:cutoff))

# ╔═╡ 6e17eb6a-f3c6-4bbb-84ff-ecade371ee42
prep = DI.prepare_gradient(loss, backend, .√w.(0:cutoff))

# ╔═╡ c8cba051-cdad-4d38-9d22-4a0acbd70769
DI.value_and_gradient(loss, prep, backend, [0.8709392376233819, 1.0775289942193647, 0.9089030511810957, 1.01707146061182, 0.9534415707659362, 0.6791021599567386, 0.8571801738805062, 0.9300761903486002, -0.6361339433542431, 0.03782761065155138])#.√w.(0:cutoff))

# ╔═╡ 0c12d10c-e02c-4c1f-9499-537b05eb20fb
let cutoff = cutoff, M = M
	μ1 = getindex.(hists2_errors[1], 2)[1:(M + 1)]
	σ1 = hypot.(getindex.(hists2_errors[1], 3)[1:(M + 1)], 0.5e-5)
	μ2 = getindex.(hists2_errors[2], 2)[1:(M + 1)]
	σ2 = hypot.(getindex.(hists2_errors[2], 3)[1:(M + 1)], 0.5e-5)
	global function loss2(wᵢ)
		w(x) = wᵢ[x + 1]^2
		l = Lanczos(w, 0:cutoff)
		kernel = map(CartesianIndices((0:cutoff, 0:cutoff))) do I
		    Kernel(l, M)(Tuple(I)...)
		end
		Y = let (λ, Q) = eigen(Symmetric(kernel))
			Q[:, λ .≈ 1]
		end
		bins = zeros(M + 1, M)
		for _ in 1:1000
			h, sens = _randDPPproj(Y)
			λ = h .- M .+ eachindex(h)
			bins[CartesianIndices.(λ .+ 1, 1:M)] .+= fake_grad.(1.0, sens)
		end
		bins ./= 1000
		
		return -sum(k -> sum(x -> logpdf(Normal(μ1[x + 1], σ1[x + 1]), bins[x + 1, k])))
		#norm(μ1 - d1) + norm(μ2 - d2)
	end
end

# ╔═╡ dc0da8f2-27b8-4f8f-b674-9f0a6b7348e8
let cutoff = cutoff, M = M, hists2_errors = hists2_errors
	global function loss3(wᵢ)
		w(x) = wᵢ[x + 1]^2
		l = Lanczos(w, 0:cutoff)
		kernel = map(CartesianIndices((0:cutoff, 0:cutoff))) do I
		    Kernel(l, M)(Tuple(I)...)
		...
		bins = zeros(M + 1, M)
		for _ in 1:10000
			h, sens = _randDPPseq!(copy(kernel))
			λ = h .- (M + 1) .+ eachindex(h)
			bins[CartesianIndex.(λ .+ 1, 1:M)] .+= fake_grad.(1.0, sens)
		end
		bins ./= 10000
		
		return -sum(k -> sum(x -> logpdf(Normal(hists2_errors[k][x + 1][2], hypot(hists2_errors[k][x + 1][3], 0.5e-5)), bins[x + 1, k]), 0:M), 1:M)
		#norm(μ1 - d1) + norm(μ2 - d2)
	end
end
...
begin
	_log10(x) = x < 0 ? -log10(floatmax()) : log10(x)
...
begin
	...
	@tasks for _ in 1:10000
		for i in 1:50
			...
			λ = Partition(YoungTableaux.ncols.(Ref(P), 1:M))
			j = DimensionalData.selectindices(dims(counts4, Dim{:λ}), At(λ))
			@atomic parent(counts4)[i, j] += 1
		end
	end
	counts4_mean = dropdims(mean(counts4 ./ 10000; dims = Dim{:i}); dims = Dim{:i})
	counts4_std = dropdims(std(counts4 ./ 10000; dims = Dim{:i}); dims = Dim{:i})
	counts4_mean, counts4_std
end

# ╔═╡ 3c55de45-59ee-4c54-83e9-9427b81b76a6
let cutoff = cutoff, M = M, counts4_mean = parent(counts4_mean), counts4_std = parent(counts4_std)
	global function loss4(wᵢ)
		w(x) = wᵢ[x + 1]^2
		l = Lanczos(w, 0:cutoff)
		kernel = map(CartesianIndices((0:cutoff, 0:cutoff))) do I
		    Kernel(l, M)(Tuple(I)...)
		...
		return -sum(enumerate(combinations(0:cutoff, M))) do (i, h)
			p = det(kernel[h .+ 1, h .+ 1])
			logpdf(Normal(counts4_mean[i], hypot(counts4_std[i], 0.5e-5)), p)
		end
	end
end

# ╔═╡ cb6dc316-5e4f-4b89-8791-c56b62c72e1d
loss4(ones(cutoff + 1))

# ╔═╡ e35c003b-d93d-440e-bacc-1fe8f59cb108
DI.value_and_gradient(loss4, backend, ones(cutoff + 1))

# ╔═╡ 6c4f2a35-4a8a-4748-8990-308c9698ef66
optf = SciMLBase.OptimizationFunction((u, p) -> loss4(u), ADTypes.AutoMooncake())

# ╔═╡ 0f24b775-f0b0-4b03-ba42-2507b25aa935
prob = SciMLBase.OptimizationProblem(optf, .√w.(0:cutoff))

# ╔═╡ 6aa081b2-1290-4338-90e7-80d4aad696a3
sol = SciMLBase.solve(prob, OptimizationOptimJL.BFGS(; manifold = OptimizationOptimJL.Sphere()), maxiters = 1000, progress = true)

# ╔═╡ e289d49b-4553-4a2c-95de-61c79dc43983
let w = sol.u.^2
	global w2(x) = w[x + 1]
end

# ╔═╡ 9e3ce14d-8134-4271-973b-73410d6ebe25
b = Lanczos(w2, 0:cutoff)

# ╔═╡ 373a530e-0d79-4fa3-bf3e-3efa8e957fcd
kernel = tmap(CartesianIndices((0:cutoff, 0:cutoff))) do I
    Kernel(b, M)(Tuple(I)...)
end

# ╔═╡ 42e633d9-3b1d-4988-8a16-a73db11c23c1
kernel^2 ≈ kernel

# ╔═╡ 0fbd2e60-b550-41e6-9726-c7da2df4535c
eigvals(kernel)

# ╔═╡ 8a509692-a690-4bd6-8a2a-fce4e94794e7
bessel ≈ kernel

# ╔═╡ b570c5f0-1007-4b8d-8822-7477e0ea7305
let colorrange = (-0.2, 1)
	fig, ax, hm = heatmap(0:cutoff, 0:cutoff, kernel; colorrange, axis = (; title = "Our Kernel"), figure = (; size = (650, 400)))
	heatmap(fig[1, 2], -M:(M - 1), -M:(M - 1), bessel; colorrange, axis = (; title = "Bessel Kernel"))
	Colorbar(fig[1, 3], hm)
	fig
end

# ╔═╡ 5dffcc01-9afb-4d36-8336-29e5ac33c03f
Y = let (λ, Q) = eigen(Symmetric(kernel))
	Q[:, λ .≈ 1]
end

# ╔═╡ 632d7267-dc1f-49bf-ba4b-69ec0ba7780c
_randDPPproj(Y)

# ╔═╡ 61ad2fab-e25c-40aa-bd2b-646968438dd0
h = randDPPproj(Y) .- 1

# ╔═╡ a8fa85b9-fb73-468a-b82e-2aea8a49ed04
...
# ╔═╡ 1fa614ae-844a-4ff8-9926-9a290f6bec1c
let
	fig = Figure()
	...
	stairs!(ax, (-1:M) .+ 0.5, @show(diff([y[(end - M - 1):end]; 1])); color = :yellow, linewidth = 2, linestyle = :dot, label = "Fredholm Det")
...
# ╔═╡ af60f778-d805-4504-9613-0a2a7f19738f
let
	fig = Figure()
	ax = Axis(fig[1, 1]; limits = (xlims, nothing))
	stairs!(ax, normalize(hists1_mean[2]); label = "DPP")
	stairs!(ax, normalize(hists2_mean[2]); color = :red, linewidth = 2, linestyle = :dash, label = "Poissonized RSK")

	y = map((cutoff - M - 1):cutoff-1) do k
		K = kernel[(k:cutoff) .+ 1, (k:cutoff) .+ 1]
		sum(k:cutoff) do t
			Kᵗ = K .- K[:, t - k + 1] .* K[t - k + 1, :]' ./ K[t - k + 1, t - k + 1]
			kernel[t + 1, t + 1] * det(I - Kᵗ)
	...
	stairs!(ax, (-1:M) .+ 0.5, @show(diff([0; y; 1])); color = :yellow, linewidth = 2, linestyle = :dot, label = "Fredholm Det")

	errorbars!(ax, hists1_errors[2] .- Vec3f(.15, 0, 0); color = Cycled(1), linewidth = 2)
...
# ╔═╡ 61a2aa58-d1b3-49fb-8741-0d67bf668bb6
for (i, h) in enumerate(combinations(0:cutoff, M))
	println("P(", h, ") = ", parent(counts4_mean)[i], " ± ", parent(counts4_std)[i])
end

# ╔═╡ 86c14832-5b9a-40ef-a78b-02aa6e198b7a
1 / (1 + θ + θ^2 / 2)

# ╔═╡ 281d017c-0485-4af9-9f31-2455a97dd9c9
12 / (12 + (24 + 24θ) * θ)

# ╔═╡ dc488756-6cc6-465e-bf18-ac7f105fade5
begin
	counts3 = zeros(Int, 2cutoff + 1, 50)
	@tasks for _ in 1:10000
		for i in 1:50
			N = rand(Poisson(θ))
			p = randperm(N)
			P = rs_norecord(p)
			X = YoungTableaux.ncols.(Ref(P), 1:M) .- (1:M)
			for j in X
				@atomic counts3[j + cutoff + 1, i] += 1
			end
		end
	end
	_counts3 = counts3 ./ 10000
	counts3_mean = vec(mean(_counts3; dims = 2))
	counts3_errors = let
		s = std(_counts3; dims = 2)
		Vec3f.(-cutoff:cutoff, vec(counts3_mean), vec(s))
	end
end

# ╔═╡ 8c441d64-304d-4d3c-aef4-d8ebe6d52d0d
let
	fig = Figure()
	ax = Axis(fig[1, 1])
	stairs!(ax, -cutoff:cutoff, counts3_mean; step = :center)
	errorbars!(ax, counts3_errors)
	stairs!(ax, -cutoff:cutoff, diag(kernel); step = :center, linestyle = :dash, linewidth = 2)
	l = axislegend(ax,
		[
			[LineElement(; color = Cycled(1)), LineElement(; color = Cycled(1), points = Point2f[(0.5, 0.2), (0.5, .8)])],
			[LineElement(; color = Cycled(2), linestyle = :dash)],
		],
		["Monte Carlo from RSK", "Diagonal Entries of K"]
	)
	fig
end
...
manifest_format = "2.0"
project_hash = "4b8a42ff946a6972ea8f6c8abbacda08df8fa96f"
...
git-tree-sha1 = "8cfcd6c39a3847cd139a981c57bb124149fa631e"
...
deps = ["Logging", "Optimisers", "OptimizationBase", "Reexport", "SciMLBase"]
git-tree-sha1 = "63737f735d5f3d2e0548ae8831ebb7cebf51758b"
